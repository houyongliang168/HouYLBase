...
import com.utils.network.NetUtils;
import com.utils.sp.SPUtils;
...
import com.yongliang.downloadmodule.downloadpage.Constant;
import com.yongliang.downloadmodule.downloadpage.activity.DownCacheActivity;
import com.yongliang.downloadmodule.downloadpage.adpter.CacheingRVAdapter;
import com.yongliang.downloadmodule.downloadpage.contract.DowningFragmentPresenter;
import com.yongliang.downloadmodule.downloadpage.contract.IDowningFragmentContract;
...
//        },200000, Permission.WRITE_EXTERNAL_STORAGE,Permission.READ_EXTERNAL_STORAGE);
    ...
    private void getFileDirectory() {
        ...
        if (!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {//无sd 卡
            if (!TextUtils.isEmpty(Environment.getExternalStorageDirectory().getPath())) {
                mFilePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separatorChar + "houyongliang"+ File.separatorChar  ;
            }
        } else {//有sd 卡
            mFilePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separatorChar + "houyongliang"+ File.separatorChar;
        }
    ...
    public boolean showNetDialog() {
        SPUtils spUtils = new SPUtils(this.getContext(),"autoLogin");
    ...
    public void setEditStatus(boolean mIsEdit){
        ...
        if(include_cache_delete==null||ll_cache==null){
            return;
        }
        if (mIsEdit) {
