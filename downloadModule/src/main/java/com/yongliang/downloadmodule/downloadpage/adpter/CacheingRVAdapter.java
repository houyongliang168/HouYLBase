...
import com.yongliang.downloadmodule.databinding.ItemCacheDoingBinding;
import com.yongliang.downloadmodule.downloadpage.bean.DownloadStatusChange;
...
public class CacheingRVAdapter extends RecyclerView.Adapter<CacheingRVAdapter.ViewHolder2> {
    ...
    public CacheingRVAdapter(Activity mContext, ObservableList<DownloadInfo> list) {
        this.mContext = mContext;
        this. mList = list;
        manager = DownloadService.getDownloadManager(mContext.getApplicationContext());
    ...
    public void onBindViewHolder(ViewHolder2 holder, final int position) {
      ...
      if(data==null){return;}
        holder.getBinding().setDownInfo(data);
...
//                }
//            });
        holder.getBinding().cbDoingEdit.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                @Override
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    data.setSelected(isChecked);
                    /* 设置是否全选的状态*/
...
//            }});
        holder.getBinding().ivDoingState.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {// 设置点击事件
                downLoadControl(data,true);
            }});

        data.setDownloadListener(new CommonDownloadListener(new SoftReference(holder)) {
                @Override
...
//        }
        ...
        public ItemCacheDoingBinding binding;
        public ViewHolder2(ItemCacheDoingBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
            iv_cache_conn_frame = (AnimationDrawable)(binding.ivDoingConn .getBackground());
        }

        public ItemCacheDoingBinding getBinding() {
            return binding;
        }

        public void refresh(DownloadInfo data, final int possion) {//刷新方法
            if (data == null||binding==null||iv_cache_conn_frame==null) {
            ...
            switch (data.getStatus()) {
                case DownloadInfo.STATUS_NONE:
                    ...
                    binding.tvDoingState.setText("连接中..");
                    binding.ivDoingConn .setVisibility(View.VISIBLE);
                    binding.progressbarDoing.setVisibility(View.GONE);
                    binding.ivDoingState.setImageResource(R.mipmap.cache_doing_pause);
                    binding.tvDoingState.setTextColor(Color.parseColor("#bab4ae"));
                    break;
                case DownloadInfo.STATUS_WAIT:
                    iv_cache_conn_frame.start();
                    binding.tvDoingState.setText("队列中..");
                    binding.ivDoingConn.setVisibility(View.VISIBLE);
                    binding.progressbarDoing.setVisibility(View.GONE);
                    binding.ivDoingState.setImageResource(R.mipmap.cache_doing_pause);
                    binding.tvDoingState.setTextColor(Color.parseColor("#bab4ae"));
                ...
                case DownloadInfo.STATUS_PAUSED:
                    ...
                    binding.tvDoingState.setTextColor(Color.parseColor("#f51706"));
                    binding.tvDoingState.setText("已暂停");
                    binding.ivDoingConn.setVisibility(View.GONE);
                    binding.progressbarDoing.setVisibility(View.VISIBLE);
                    binding.ivDoingState.setImageResource(R.mipmap.cache_doing_down);
                    break;
                case DownloadInfo.STATUS_ERROR:
                    iv_cache_conn_frame.stop();
                    binding.tvDoingState.setText("下载失败");
                    binding.ivDoingConn.setVisibility(View.GONE);
                    binding.progressbarDoing.setVisibility(View.VISIBLE);
                    binding.tvDoingState.setTextColor(Color.parseColor("#f51706"));
                    binding.ivDoingState.setImageResource(R.mipmap.cache_doing_refresh);
                    break;
                case DownloadInfo.STATUS_DOWNLOADING:
                    iv_cache_conn_frame.stop();
                    binding.ivDoingConn.setVisibility(View.GONE);
                    ...
                    if (countLen < 1) {
                        ...
                        binding.tvDoingState.setText(String.format("%.2f%s%.2f%s", countLen, "K/", readLen, "KB"));
                    } else {
                        binding.tvDoingState.setText(String.format("%.2f%s%.2f%s", countLen, "M/", readLen, "MB"));
                    }
                    binding.ivDoingState.setImageResource(R.mipmap.cache_doing_pause);
                    binding.progressbarDoing.setMax((int) countLenF);
                    binding.progressbarDoing.setProgress((int) readLenF);
                    break;
                case DownloadInfo.STATUS_PREPARE_DOWNLOAD:
                    iv_cache_conn_frame.start();
                    binding.tvDoingState.setText("连接中..");
                    ...
                    break;
                case DownloadInfo.STATUS_COMPLETED:
                    binding.tvDoingState.setText("下载完成");
                    binding.progressbarDoing.setVisibility(View.VISIBLE);
                    binding.tvDoingState.setTextColor(Color.parseColor("#f51706"));
                    ...
                    binding.getRoot().post(new Runnable() {
                        @Override
                        public void run() {
                            //解决Cannot call this method while RecyclerView is computing a layout or scrolling android.support.v7.widget.RecyclerView
