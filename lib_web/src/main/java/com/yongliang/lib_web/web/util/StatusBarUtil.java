...
/**
 * setSystemUiVisibility() 设置系统UI元素的可见性
 * SYSTEM_UI_FLAG_FULLSCREEN 表示全屏的意思，也就是会将状态栏隐藏
 * SYSTEM_UI_FLAG_LAYOUT_STABLE  设置后,状态栏有时间
 * ActionBar的hide()方法将ActionBar也进行隐藏
 *
 *  getWindow().setStatusBarColor(Color.TRANSPARENT); 状态栏 设置为 透明
 *
 *  SYSTEM_UI_FLAG_HIDE_NAVIGATION   隐藏状态栏和导航栏,触摸屏幕的任意位置都会退出全屏
 *  需要设置:
...
public class StatusBarUtil {
    ...
    // 设置透明状态栏
    public static void  setOwnTranslucent(@NonNull Window window){
...
//        int height = 0;
//        try {
//            int resourceId = App.getApp().getResources().getIdentifier("status_bar_height", "dimen",
//                    "android");
//            if (resourceId > 0) {
//                height = App.getApp().getResources().getDimensionPixelSize(resourceId);
//            }
//        } catch (Exception e) {
//            e.printStackTrace();
//            MyLog.wtf(TAG,e.getMessage()+"");//打印日志信息
//        }
//        return height;
//
//    }
    ...
    public int getStatusBarHeight4(Activity activity){
    Rect rect = new Rect();
    ...
    public static void setColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int statusBarAlpha) {
        //21以上
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);// 使内容进入状态栏
...
//        }
    ...
    private static void transparentStatusBar(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);//使內容進入導航欄
