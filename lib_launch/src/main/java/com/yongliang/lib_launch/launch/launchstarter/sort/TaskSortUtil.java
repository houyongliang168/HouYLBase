...
public class TaskSortUtil {
    ...
    private static List<Task> sNewTasksHigh = new java.util.ArrayList<> ();// 高优先级的Task
    ...
    public static synchronized List<Task> getSortResult(List<Task> originTasks,
        ...
        java.util.Set<Integer> dependSet = new androidx.collection.ArraySet<> ();
    ...
    private static List<Task> getResultTasks(List<Task> originTasks,
                                             java.util.Set<Integer> dependSet, List<Integer> indexList) {
        List<Task> newTasksAll = new java.util.ArrayList<> (originTasks.size());
        List<Task> newTasksDepended = new java.util.ArrayList<> ();// 被别人依赖的
        List<Task> newTasksWithOutDepend = new java.util.ArrayList<> ();// 没有依赖的
        List<Task> newTasksRunAsSoon = new java.util.ArrayList<> ();// 需要提升自己优先级的，先执行（这个先是相对于没有依赖的先）
